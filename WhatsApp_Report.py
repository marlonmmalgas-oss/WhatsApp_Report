...
with st.expander("Idle Entries"):
    for i in range(num_idles):
        ...
        selected_delay = st.selectbox(f"Select Delay {i+1}", options=idle_options, key=f"delay_select_{i}")
...
# Keep a separate 4-hour count for updates or corrections
if "four_hour_count" not in st.session_state:
    st.session_state.four_hour_count = {
...
# Update counts manually if inputs are changed
st.session_state.four_hour_count["load"]["FWD"] = fwd_4h_load
st.session_state.four_hour_count["load"]["MID"] = mid_4h_load
st.session_state.four_hour_count["load"]["AFT"] = aft_4h_load
st.session_state.four_hour_count["load"]["POOP"] = poop_4h_load

st.session_state.four_hour_count["disch"]["FWD"] = fwd_4h_disch
st.session_state.four_hour_count["disch"]["MID"] = mid_4h_disch
st.session_state.four_hour_count["disch"]["AFT"] = aft_4h_disch
st.session_state.four_hour_count["disch"]["POOP"] = poop_4h_disch

st.session_state.four_hour_count["restow_load"]["FWD"] = fwd_4h_restow_load
st.session_state.four_hour_count["restow_load"]["MID"] = mid_4h_restow_load
st.session_state.four_hour_count["restow_load"]["AFT"] = aft_4h_restow_load
st.session_state.four_hour_count["restow_load"]["POOP"] = poop_4h_restow_load

st.session_state.four_hour_count["restow_disch"]["FWD"] = fwd_4h_restow_disch
st.session_state.four_hour_count["restow_disch"]["MID"] = mid_4h_restow_disch
st.session_state.four_hour_count["restow_disch"]["AFT"] = aft_4h_restow_disch
st.session_state.four_hour_count["restow_disch"]["POOP"] = poop_4h_restow_disch
# Part 5/5
# --- 4-Hourly WhatsApp Template and Send Section ---
st.header("📤 Send 4-Hourly Report to WhatsApp")

# Calculate cumulative totals for 4-hourly report from manual counts
four_hour_done_load = sum(st.session_state.four_hour_count["load"].values())
four_hour_done_disch = sum(st.session_state.four_hour_count["disch"].values())
four_hour_done_restow_load = sum(st.session_state.four_hour_count["restow_load"].values())
four_hour_done_restow_disch = sum(st.session_state.four_hour_count["restow_disch"].values())

# Remaining totals
four_hour_remain_load = cumulative["planned_load"] - cumulative["done_load"] - four_hour_done_load
four_hour_remain_disch = cumulative["planned_disch"] - cumulative["done_disch"] - four_hour_done_disch
four_hour_remain_restow_load = cumulative["planned_restow_load"] - cumulative["done_restow_load"] - four_hour_done_restow_load
four_hour_remain_restow_disch = cumulative["planned_restow_disch"] - cumulative["done_restow_disch"] - four_hour_done_restow_disch

# Generate 4-hourly WhatsApp template with monospace formatting
...
*CRANE MOVES*
           Load    Disch
FWD       {st.session_state.four_hour_count["load"]["FWD"]:>5}     {st.session_state.four_hour_count["disch"]["FWD"]:>5}
MID       {st.session_state.four_hour_count["load"]["MID"]:>5}     {st.session_state.four_hour_count["disch"]["MID"]:>5}
AFT       {st.session_state.four_hour_count["load"]["AFT"]:>5}     {st.session_state.four_hour_count["disch"]["AFT"]:>5}
POOP      {st.session_state.four_hour_count["load"]["POOP"]:>5}     {st.session_state.four_hour_count["disch"]["POOP"]:>5}
_________________________
*RESTOWS*
           Load    Disch
FWD       {st.session_state.four_hour_count["restow_load"]["FWD"]:>5}     {st.session_state.four_hour_count["restow_disch"]["FWD"]:>5}
MID       {st.session_state.four_hour_count["restow_load"]["MID"]:>5}     {st.session_state.four_hour_count["restow_disch"]["MID"]:>5}
AFT       {st.session_state.four_hour_count["restow_load"]["AFT"]:>5}     {st.session_state.four_hour_count["restow_disch"]["AFT"]:>5}
POOP      {st.session_state.four_hour_count["restow_load"]["POOP"]:>5}     {st.session_state.four_hour_count["restow_disch"]["POOP"]:>5}
_________________________
*CUMULATIVE (from hourly totals)*
           Load   Disch
Plan       {cumulative["planned_load"]:>5}      {cumulative["planned_disch"]:>5}
Done       {cumulative["done_load"] + four_hour_done_load:>5}      {cumulative["done_disch"] + four_hour_done_disch:>5}
Remain     {four_hour_remain_load:>5}      {four_hour_remain_disch:>5}
_________________________
*Restows*
           Load   Disch
Plan       {cumulative["planned_restow_load"]:>5}      {cumulative["planned_restow_disch"]:>5}
Done       {cumulative["done_restow_load"] + four_hour_done_restow_load:>5}      {cumulative["done_restow_disch"] + four_hour_done_restow_disch:>5}
Remain     {four_hour_remain_restow_load:>5}      {four_hour_remain_restow_disch:>5}
_________________________
*Hatch Moves*
             Open      Close
...
# Optional: Reset 4-hourly counters
