...
def default_cumulative():
    return {
        ...
        "first_lift": "",
        "last_lift": "",
        # non-persistent hourly-only field like gearbox will not be stored here as persistent,
        # but we'll store latest gearbox in cumulative for immediate template display (hourly only)
        "gearbox": 0,
    }
...
def init_db():
    ...
    cur = conn.cursor()
    # simple key/value table for JSON blobs
    cur.execute("""
    ...
    if not row:
        ...
        conn.commit()
    # ensure fourh key exists
    cur.execute("SELECT value FROM meta WHERE key='fourh';")
    row = cur.fetchone()
    if not row:
        cur.execute("INSERT INTO meta (key, value) VALUES ('fourh', ?);", (json.dumps(empty_tracker()),))
        conn.commit()
    conn.close()

def load_db():
    # returns (cumulative: dict, fourh: dict)
    init_db()  # make sure DB and keys exist
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute("SELECT value FROM meta WHERE key='cumulative';")
    row = cur.fetchone()
...
def save_db(cumulative: dict, fourh: dict = None):
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute("INSERT OR REPLACE INTO meta (key, value) VALUES ('cumulative', ?);", (json.dumps(cumulative),))
...
cumulative_loaded, fourh_loaded = load_db()

# keep the loaded cumulative and fourh in session state for quick access
init_key("cumulative", cumulative_loaded)
init_key("fourh", fourh_loaded)

# date & labels
init_key("report_date", datetime.now(TZ).date())
# basic fields mapped from loaded cumulative (do not overwrite if user edits later)
...
# plans & openings (from persisted data, editable in UI)
for k in [
    ...
    init_key(k, st.session_state["cumulative"].get(k, default_cumulative()[k]))

# HOURLY inputs (hourly-only values)
for k in [
...
# FOUR-HOUR manual override keys
for k in [
...
with left:
    ...
    # DO NOT assign widget return to session_state; use keys so edits persist
    st.text_input("Vessel Name", key="vessel_name")
    st.text_input("Berthed Date", key="berthed_date")
    st.text_input("First Lift (time/crane)", key="first_lift", help="Optional: first lift details")
    st.text_input("Last Lift (time/crane)", key="last_lift", help="Optional: last lift details")
with right:
...
# --------------------------
# Gearbox (hourly-only, appears on template for that hour)
# --------------------------
with st.expander("🧰 Gearbox (Hourly only)"):
    st.number_input("Gearbox Total (this hour)", min_value=0, key="gearbox", help="Hourly only; will reset on hourly reset/generate")
...
# --------------------------
# WhatsApp (Hourly) – original monospace template (kept as-is)
# --------------------------
...
def generate_hourly_template_from_state(cum):
    # cum is the cumulative dict (after update)
    remaining_load  = st.session_state["planned_load"]  - cum["done_load"]
    remaining_disch = st.session_state["planned_disch"] - cum["done_disch"]
    remaining_restow_load  = st.session_state["planned_restow_load"]  - cum["done_restow_load"]
    remaining_restow_disch = st.session_state["planned_restow_disch"] - cum["done_restow_disch"]
...
def reset_master():
    # master reset clears DB and session-state to defaults
    try:
        if os.path.exists(DB_FILE):
            os.remove(DB_FILE)
    except Exception:
        pass
    # re-init DB and session state
    init_db()
    cum, fourh = load_db()
    ...
    for k in [
        ...
        st.session_state[k] = st.session_state["cumulative"].get(k, default_cumulative()[k])
    reset_hourly_inputs()
    reset_4h_tracker()

# --------------------------
# 4-hour computed/manual helpers
# --------------------------
def computed_4h():
    tr = st.session_state["fourh"]
    return {
...
_________________________
           Load   Disch
Plan       {st.session_state['planned_load']:>5}      {st.session_state['planned_disch']:>5}
Done       {st.session_state['cumulative']['done_load']:>5}      {st.session_state['cumulative']['done_disch']:>5}
Remain     {remaining_load:>5}      {remaining_disch:>5}
_________________________
*Restows*
           Load    Disch
Plan       {st.session_state['planned_restow_load']:>5}      {st.session_state['planned_restow_disch']:>5}
Done       {st.session_state['cumulative']['done_restow_load']:>5}      {st.session_state['cumulative']['done_restow_disch']:>5}
Remain     {remaining_restow_load:>5}      {remaining_restow_disch:>5}
_________________________
*Hatch Moves*
             Open         Close
...
# --------------------------
# Generate hourly & update cumulative — single button (no preview)
# --------------------------
def on_generate_hourly():
    cum = st.session_state["cumulative"]

    # apply opening balances once (so they appear in Done and affect Remain)
    ...
    # add to cumulative
    cum["done_load"] += hour_load
    cum["done_disch"] += hour_disch
    cum["done_restow_load"] += hour_restow_load
    cum["done_restow_disch"] += hour_restow_disch
    cum["done_hatch_open"] += hour_hatch_open
    cum["done_hatch_close"] += hour_hatch_close
    # store gearbox latest for template display (hourly only)
    cum["gearbox"] = int(st.session_state.get("gearbox",0))

    # ensure Done never greater than Plan visible (if done > plan, adjust plan to match done)
    if cum["done_load"] > st.session_state["planned_load"]:
        st.session_state["planned_load"] = cum["done_load"]
    if cum["done_disch"] > st.session_state["planned_disch"]:
        st.session_state["planned_disch"] = cum["done_disch"]
    if cum["done_restow_load"] > st.session_state["planned_restow_load"]:
        st.session_state["planned_restow_load"] = cum["done_restow_load"]
    if cum["done_restow_disch"] > st.session_state["planned_restow_disch"]:
        st.session_state["planned_restow_disch"] = cum["done_restow_disch"]

    # persist meta/settings into cumulative (so they survive across devices)
    cum.update({
    ...
    # auto-advance hour NEXT render safely
    st.session_state["hourly_time_override"] = next_hour_label(st.session_state["hourly_time"])

    # return template text (already using updated cumulative)
    return generate_hourly_template_from_state(cum)

# Buttons: single generate + open whatsapp (uses same generation)
...
st.button("🔄 Reset 4-Hourly Tracker (clear last 4 hours)", on_click=reset_4h_tracker)
st.button("⚠️ Master Reset (Everything)", on_click=reset_master)

# show 4-hour template preview / send
...
st.caption(
    ...
    "• 4-Hourly: Use **Populate 4-Hourly from Hourly Tracker** if needed; Manual Override also available. "
